...
internal class RoaringArray : IEnumerable<int>, IEquatable<RoaringArray> {
    ...
    internal Container? GetContainer(ushort x) {
        int i = BinarySearch(0, _size, x);
        if (i < 0) {
            return null;
        }
        return _values[i];
    ...
    private int BinarySearch(int begin, int end, ushort key) {
        return Util.UnsignedBinarySearch(_keys, begin, end, key);
    }
    ...
    private int AdvanceUntil(ushort key, int index) {
        return Util.AdvanceUntil(_keys, index, _keys.Length, key);
    }
